...
import 'package:nilam/core/utils/utils.dart';
...
class SliderB extends StatefulWidget {
  const SliderB({required this.slideItems, super.key});
  final List<BannerResponse> slideItems;

  @override
  State<SliderB> createState() => _SliderBState();
}

class _SliderBState extends State<SliderB> {
  int current = 0;
        ...
        CarouselSlider(
          ...
          items: widget.slideItems.map((item) {
            return SizedBox(
              ...
              height: size.width / 2,
              child:
                  NetWorkImageViewB(imageUrl: baseUrl + item.mobileImageUrl!),
            );
          }).toList(),
